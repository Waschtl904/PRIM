...
class AdvancedBenchmarkAnalyzer:
    ...
    def __init__(self, use_numba: bool = True, use_parallel: bool = False, use_cache: bool = True):
    ...
    def benchmark_sieve_methods(self, limit: int, segment_sizes: List[int]) -> Dict[str, Any]:
    ...
    def run_parameter_grid_benchmark(self, 
                                   limits: List[int], 
                                   segment_sizes: List[int],
                                   numba_configs: List[Dict[str, bool]]) -> pd.DataFrame:
        ...
        print(f"Starte Parameter-Grid-Benchmark mit {total_combinations} Konfigurationen...")
        ...
        for limit in limits:
            for segment_size in segment_sizes:
                for config in numba_configs:
                    ...
                    try:
                        # Sieb-Benchmark
                        sieve_results = analyzer.benchmark_sieve_methods(limit, [segment_size])
                        
                        # Test-Zahlen für Primzahltests generieren (letzte 100 aus dem Sieb)
                        if 'numba_standard' in sieve_results:
                            # Verwende Standard-Sieb für Test-Zahlen
                            test_primes, _, _ = analyzer.measure_with_memory(analyzer.sieve_numba, min(10000, limit))
                            test_numbers = test_primes[-100:].tolist() if len(test_primes) >= 100 else test_primes.tolist()
                        else:
                            # Fallback: einfache Test-Zahlen
                            test_numbers = list(range(max(2, limit-200), limit, 2))[:100]
                        ...
                        for method, data in sieve_results.items():
                            result_row = {
                                ...
                                'fj32_runtime': primality_results.get('fj32_c', {}).get('runtime', 
                                                primality_results.get('fj32_fallback', {}).get('runtime', 0)),
                                'fj32_method': 'fj32_c' if 'fj32_c' in primality_results else 'fj32_fallback'
                            }
    ...
    def create_visualizations(self, df: pd.DataFrame) -> None:
        ...
        for method in df['method'].unique():
            ...
            ax1.loglog(method_data['limit'], method_data['sieve_runtime'], 
        ...
        for method in df['method'].unique():
            method_data = df[df['method'] == method]
            ax2.loglog(method_data['limit'], method_data['sieve_memory_mb'], 
        ...
        for fj32_method in df['fj32_method'].unique():
            ...
            ax3.loglog(method_data['limit'], method_data['fj32_runtime'], 
                      marker='^', label=fj32_method, alpha=0.7)
        ...
        for method in df['method'].unique():
            method_data = df[df['method'] == method]
            ax4.loglog(method_data['limit'], method_data['primes_per_second'], 
        ...
        plt.tight_layout()
        plt.savefig('plots/modul4_performance_analysis.png', dpi=300, bbox_inches='tight')
        plt.show()
        ...
        if len(df['segment_size'].dropna().unique()) > 1:
            ...
            # Runtime Heatmap
            pivot_runtime = df[df['method'].str.contains('segmented', na=False)].pivot_table(
                values='sieve_runtime', index='segment_size', columns='limit', aggfunc='mean'
            )
            ...
            # Memory Heatmap
            pivot_memory = df[df['method'].str.contains('segmented', na=False)].pivot_table(
                values='sieve_memory_mb', index='segment_size', columns='limit', aggfunc='mean'
            )
    ...
    def export_results(self, df: pd.DataFrame, filename_base: str = "modul4_results") -> None:
        ...
        with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
            ...
            summary_by_method = df.groupby(['method', 'limit']).agg({
            ...
            best_configs.to_excel(writer, sheet_name='Beste_Konfigurationen', index=False)
    ...
    def _generate_recommendations(self, df: pd.DataFrame, filename_base: str) -> None:
        ...
        recommendations.append(f"Generiert am: {pd.Timestamp.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        ...
        for limit in sorted(df['limit'].unique()):
            ...
            recommendations.append(f"- **Beste Laufzeit**: {best_runtime['method']} "
                                 f"({best_runtime['sieve_runtime']:.6f}s, "
                                 f"{best_runtime['sieve_memory_mb']:.2f}MB)\n")
            recommendations.append(f"- **Geringster Speicher**: {best_memory['method']} "
        ...
        if df['parallel'].any():
            parallel_avg = df[df['parallel'] == True]['sieve_runtime'].mean()
            serial_avg = df[df['parallel'] == False]['sieve_runtime'].mean()
            if parallel_avg < serial_avg:
                recommendations.append("✓ **Numba Parallelisierung** verbessert die Performance signifikant\n")
            else:
                recommendations.append("⚠ **Numba Parallelisierung** zeigt keinen deutlichen Vorteil\n")
        
        # Segmentierung Analyse
        segmented_data = df[df['method'].str.contains('segmented', na=False)]
        if not segmented_data.empty:
            best_segments = segmented_data.groupby('segment_size')['sieve_runtime'].mean().sort_values()
            recommendations.append(f"✓ **Optimale Segment-Größe**: {best_segments.index[0]:,} "
...
def main():
    ...
    with warnings.catch_warnings():
        ...
        results_df = analyzer.run_parameter_grid_benchmark(limits, segment_sizes, numba_configs)
    ...
    analyzer.export_results(results_df, f"modul4_benchmark_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}")
